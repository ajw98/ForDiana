...
Model_Energy_DUN :: Model_Energy_DUN( Operators_Base & OpObj, int ncomp, int nicomp ):
...
Model_Energy_DUN :: ~Model_Energy_DUN()
{ // {{{
} // }}}

// ----------------- Input/Output -----------------
void Model_Energy_DUN :: set_params()
{ // {{{
  ...
  if (jsonFile("params_EnergyModel.in"))
  {
    ...
    if ( f2.is_open() )
    {
      ...
      rapidjson::Value& kappa_in = d["Kappa"];
      for( int i=0; i < _NIcomp; i++ )
      {
        _Kappa[i] = kappa_in[i].GetDouble();
      }

      _KappaM = kappa_in[_Ncomp-1].GetDouble();
...
void Model_Energy_DUN::calc_mu(const SmartFieldVec & phi, SmartFieldVec & mu)
{ // {{{ 
  ...
  {
    std::cout << "*** Error in Model_Energy_DUN::calc_mu ***\n";
  ...
  FieldType phi1_mean = phi[1].getaverage();
  ...
  _Coul_tau *= (_C[1][1]/(phi1_mean*_alpha[1]*_alpha[1]*4.));
  mu[1] += _Coul_tau;
  mu[2] -= _Coul_tau; 

  // Alternative gradient terms formulation
  /*SmartFieldVec logphi(_NIcomp), grad2_logphi(_NIcomp), del_logphi_2(_NIcomp);
  SmartField logphiM, grad2_logphiM, del_logphiM_2;
  logphi.setflag_inrealspace( false );
  grad2_logphi.setflag_inrealspace( false );
  del_logphi_2.setflag_inrealspace( false );
  logphiM.setflag_inrealspace( false );
  grad2_logphiM.setflag_inrealspace( false );
  del_logphiM_2.setflag_inrealspace( false );
  ...
  tmp.fft();
  logphiM = tmp;
  _OpObj->Del2_f_ex( logphiM, grad2_logphiM );
  _OpObj->Del_f_2_ex( logphiM, del_logphiM_2 );
  tmp2 = grad2_logphiM;
  tmp2 *= FieldType(2.);
  tmp2 += del_logphiM_2;
  ...
  for (int i = 0; i < _NIcomp; i++)
  {
    ...
    tmp.fft();
    logphi[i] = tmp;
    _OpObj->Del2_f_ex( logphi[i], grad2_logphi[i] );
    _OpObj->Del_f_2_ex( logphi[i], del_logphi_2[i] );
  ...
  // The square root form from Doi-Uneyama is calculated below
  // Use this block instead of the above one if you need it

  /*SmartField tau1, tau2, Coul_tau1, Coul_tau2;
  tau1.setflag_inrealspace(false);
  tau2.setflag_inrealspace(false);
  Coul_tau1.setflag_inrealspace(false);
  Coul_tau2.setflag_inrealspace(false);
  tmp1.sqrt(phi_r[1]);
  tmp2.sqrt(phi_r[2]);
  ...
  _OpObj->Coulomb_f_ex( tau1, Coul_tau1 );
  _OpObj->Coulomb_f_ex( tau2, Coul_tau2 );

  tmp1 = Coul_tau1;
  tmp1 *= _C[1][1]/(8*_alpha[1]*_alpha[1]);  
  mu[1] -= tmp1;

  tmp2 = Coul_tau2;
  tmp2 *= _C[2][2]/(8*_alpha[2]*_alpha[2]);
  mu[2] -= tmp2;*/
...
void Model_Energy_DUN::calc_mu_lin_exp(const SmartFieldVec & phi, SmartFieldVec & mu_lin)
{ // {{{ 
  ...
  if (phi.getflag_inrealspace() != false)
  {
    std::cout << "*** Error in Model_Energy_DUN::calc_mu_lin_exp ***\n";
    std::cout << "*** phi must be in Fourier space ***\n";
  }
...
void Model_Energy_DUN::calc_mu_lin_imp(const SmartFieldVec & phi, SmartFieldOpMat & F)
{ // {{{ 

  if (_PrecalcMuImp==false)
  {
    std::cout << "*** Error in Model_Energy_DUN::calc_mu_lin_imp ***\n";
  ...
  for (int i = 0; i < _NIcomp; i++)
  {
    for (int j = 0; j < _NIcomp; j++)
    {
      CI[i][j] = (_C[i][j] - _C[i][_NIcomp] - _C[_NIcomp][j])/(phi1_mean*_alpha[i]*_alpha[i]*4.);
    }
  }
...
void Model_Energy_DUN::calc_Hessian( const SmartFieldVec & phi, SmartFieldMat & Hess )
{ // {{{

  if (phi.getflag_inrealspace() != true)
  {
    std::cout << "*** Error in Model_Energy_DUN::calc_Hmax ***\n";
  ...
  // Precalculate 1/(alpha_M phi_M) and 1/sqrt(phi_1 phi_2)/4
  ...
  for (int i = 0; i < _NIcomp; i++)
  {
    ...
    // H_11 += 1/sqrt(phi_1 phi_2)/4  phi_2/phi_1/alpha_1
    // H_22 += 1/sqrt(phi_1 phi_2)/4  phi_1/phi_2/alpha_2

    if (i == 1)
      Hess(i,i) = (0.4015/_alpha[i]);
    else if (i == 2)
      Hess(i,i) = (0.4015/_alpha[i]);
    else
      Hess(i,i) = (1./_alpha[i]);
    ...
    if (i == 1)
    {
      ...
      tmp3 *= phi[2];
      tmp3 /= phi[1];
      Hess(i,i).xpby_inplace(tmp3,(1./_alpha[i]));   
    }
    ...
    for (int j=i+1; j < _NIcomp; j++)
    {
      ...
      // H_12, H_21 -= 1/sqrt(phi_1 phi_2)/(4 alpha_2)
      Hess(i,j) = tmp1;
      Hess(i,j) += _ChiN_ijM[i][j];
      if (i == 1)
        if (j == 2)
          Hess(i,j).xpby_inplace(tmp2,(-1./_alpha[2]));
...
Vec2dFieldType Model_Energy_DUN::calc_Hmax( const SmartFieldVec & phi )
{ // {{{
...
void Model_Energy_DUN::calc_del_mu(const SmartFieldVec & phi, SmartFieldMat & del_mu)
{ // {{{ 
  ...
  if (phi.getflag_inrealspace() != false)
  {
    std::cout << "*** Error in Model_Energy_DUN::calc_del_mu ***\n";
    std::cout << "*** phi must be in Fourier space ***\n";
  }

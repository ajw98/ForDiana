...
TimeInt_ModelH_DelMu :: TimeInt_ModelH_DelMu( int Ncomp, int NIcomp,
                                  Grid* CurrGrid ):
...
// {{{
{
  ...
  std::cout << "   - Model H (convection/diffusion) with Del_Mu terms" << std::endl;
...
TimeInt_ModelH_DelMu :: ~TimeInt_ModelH_DelMu()
{ // {{{
} // }}}

// ----------------- Setup/Cleanup -----------------

void TimeInt_ModelH_DelMu :: setup_operators()
{ // {{{
  ...
  // set up the BCs and operators for phi
  {
    ...
    if (jsonFile(filename.c_str()))
    {
      ...
      {
        std::cout << "*** Error in TimeInt_ModelH_DelMu :: setup_operators() ***\n";
...
void TimeInt_ModelH_DelMu :: cleanup_operators()
{ // {{{
...
void TimeInt_ModelH_DelMu :: set_params()
{//{{{
...
void TimeInt_ModelH_DelMu::read_initial_state()
{ // {{{
...
void TimeInt_ModelH_DelMu::write_final_state()
{ // {{{
...
void TimeInt_ModelH_DelMu :: write_curr_state( int n_disp )
{ // {{{
...
void TimeInt_ModelH_DelMu :: outer_loop()
{ // {{{
  ...
  while( t < _TMax && Nt < _NtMax )
  {
    ...
    while( phi_err >= _PhiErrTol && phi_iter <= _PhiIterMax )
    {
      ...
      while (!is_physical)
      {
        ...
        phi_half = _Phi;
        step_phi( phi_half, _Vel, del_mu_current, mu_current_linexp, mu_current_linimp, dt/2., t );
        ...
        { 
          ...
          {
            std::cout << "*** (ERROR) TimeInt_ModelH_DelMu::outer_loop ***\n";
        ...
        // Solve v
        step_vel( phi_half, vel_half, del_mu_half, vel_iter, t );
        // Update slope = 2*(vel_half-_Vel)/dt
        vel_slope = vel_half;
        vel_slope.axpby_inplace(_Vel, 2./dt, -2./dt);

        // second half step for phi
        step_phi( phi_half, vel_half, del_mu_half, mu_half_linexp, mu_half_linimp, dt/2., t );
        is_physical = check_phi(phi_half,_GlassMax);
        if ( !is_physical ) 
        { 
          if (_VarDtFlag)
        ...
        // whole step for phi
        phi_whole = _Phi;
        step_phi( phi_whole, _Vel, del_mu_current, mu_current_linexp, mu_current_linimp, dt, t ); 
    ...
    // Update _Vel
    step_vel( _Phi, _Vel, del_mu_half, vel_iter, t );
...
void TimeInt_ModelH_DelMu :: step_phi( SmartFieldVec &phi, 
...
{ // {{{
  ...
  _PhiOp->Del_A_Del_f_ex( del_mu, _Mobility, _tmpvec ); // tmpvec <- del.(Mij del. mu_n)
  rhs.xpby_inplace(_tmpvec, dt);
...
void TimeInt_ModelH_DelMu :: step_vel( SmartFieldVec const &phi, 
                                 SmartFieldVec &vel, 
                                 SmartFieldMat const &del_mu,
                                 UInt &iter,
                                 RealType t )
...
RealType TimeInt_ModelH_DelMu :: update_timestep( SmartFieldVec & vel, 
                                            RealType phi_err, 
                                            RealType dt )
{ // {{{

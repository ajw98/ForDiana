...
BCs_TimeDep_Phi :: BCs_TimeDep_Phi(std::string filename, int nicomp, 
                       Grid * gridarg ) : 
...
{ // {{{
  ...
  std::string timedep_file("params_BCs_phi_time.in");
  ...
  if ( f2.is_open() )
  {
    ...
    _BC_xeq0.resize(_NBCs, Vec2dReal(_NIcomp, Vec1dReal(_Nt, 0.)));
    for ( UInt i=0; i<_NBCs; i++)
    {
      for ( UInt j=0; j<_NIcomp; j++)
      {
        std::getline(f2, tmp_str); // header
        for ( UInt k=0; k<_Nt; k++)
        {
          std::getline(f2, tmp_str);
          _BC_xeq0[i][j][k] = atof(tmp_str.c_str());
        }
        std::getline(f2, tmp_str); // extra space
      }
    }

    _BC_xeqL.resize(_NBCs, Vec2dReal(_NIcomp, Vec1dReal(_Nt, 0.)));
    for ( UInt i=0; i<_NBCs; i++)
    {
      for ( UInt j=0; j<_NIcomp; j++)
      {
        ...
        {
          std::getline(f2, tmp_str);
          _BC_xeqL[i][j][k] = atof(tmp_str.c_str());
        }
        std::getline(f2, tmp_str); // extra space
      }
    }
  ...
  for (int i=0; i<_NBCs; ++i)
  {
    for (int j=0; j<_NIcomp; ++j)
    {
      std::cout << "      BC_xeq0[" << i << "][" << j <<"][t=0]";
      std::cout <<" = "<< _BC_xeq0[i][j][0] << "\n";
    }
  }
  for (int i=0; i<_NBCs; ++i)
  {
    for (int j=0; j<_NIcomp; ++j)
    {
      std::cout << "      BC_xeqL[" << i << "][" << j <<"][t=0]";
      std::cout <<" = "<< _BC_xeqL[i][j][0] << "\n";
    }
  }
} // }}}

BCs_TimeDep_Phi :: ~BCs_TimeDep_Phi()
{ // {{{
} // }}}

// ----------------- Constant BCs --------------------

void BCs_TimeDep_Phi::solve_BCs( SmartFieldOpMat & T, 
                              SmartFieldVec & rhs, 
                              SmartFieldVec & x,
                              RealType t )
{ // {{{
...
void BCs_TimeDep_Phi::calc_BCs( MatInterFieldMat & T, VecInterFieldVec & b, RealType t )
{ // {{{
  ...
  for (int i=0; i<_NBCs; i++) // row where we will install the equation
  {
    ...
    for (int k=0; k<_NIcomp; k++) 
    {
      tmpvec[k] = linterp(t, _BC_times, _BC_xeq0[i][k]);
    }
    tmpvec.fft();
    b[i] = tmpvec;
  ...
  for (int i=Nx-_NBCs; i<Nx; i++)
  {
    ...
    for (int k=0; k<_NIcomp; k++) 
    {
      tmpvec[k] = linterp(t, _BC_times, _BC_xeqL[Nx-_NBCs-i+1][k]);
    }
    tmpvec.fft();
    b[i] = tmpvec;

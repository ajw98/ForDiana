...
TimeInt_ModelH_GridDef :: TimeInt_ModelH_GridDef( int Ncomp, int NIcomp,
                                  Grid* CurrGrid ):
...
// {{{
{
  ...
  std::cout << "   - Model H (convection/diffusion) with Grid Deformation" << std::endl;
...
TimeInt_ModelH_GridDef :: ~TimeInt_ModelH_GridDef()
{ // {{{
} // }}}

// ----------------- Setup/Cleanup -----------------

void TimeInt_ModelH_GridDef :: setup_operators()
{ // {{{
  ...
  // set up the BCs and operators for phi
  {
    ...
    if (jsonFile(filename.c_str()))
    {
      ...
      {
        std::cout << "*** Error in TimeInt_ModelH_GridDef :: setup_operators() ***\n";
...
void TimeInt_ModelH_GridDef :: cleanup_operators()
{ // {{{
...
void TimeInt_ModelH_GridDef :: set_params()
{//{{{
  ...
  if (jsonFile("params_TimeInt.in"))
  {
    if ( f1.is_open() )
    {
      ...
      _G.resize( _Dim, Vec1dReal( _Dim, 0.));
      _gammadot = 0.;
      RealType gammadotsqr = 0.;
      rapidjson::Value& G_in = d["G"];
      for( UInt i=0; i<_Dim; i++ )
      {
        rapidjson::Value& G_in_2 = G_in[i];
        for ( UInt j=0; j<_Dim; j++ )
        {
          _G[i][j] = G_in_2[j].GetDouble();

          gammadotsqr += _G[i][j] * _G[i][j];
        }
      }
      _gammadot = std::sqrt(gammadotsqr);

      _CellTensor.resize( _Dim, Vec1dReal( _Dim, 0.));
      for( UInt i=0; i<_Dim; i++ )
        _CellTensor[i][i] = 1.;

      _InitBoxTensor.resize( _Dim, Vec1dReal(_Dim, 0.));
      _InitBoxTensor = _CurrGrid->GetCell()->getBoxTensor();
...
void TimeInt_ModelH_GridDef::read_initial_state()
{ // {{{
...
void TimeInt_ModelH_GridDef::write_final_state()
{ // {{{
...
void TimeInt_ModelH_GridDef :: write_curr_state( int n_disp )
{ // {{{
...
void TimeInt_ModelH_GridDef :: outer_loop()
{ // {{{

  SmartFieldVec phi_half( _Phi );
  SmartFieldVec phi_whole( _Phi );
  SmartFieldVec vel_half( _Vel );
  SmartFieldVec vel_guess( _Vel );
  SmartFieldVec vel_slope( _Vel );

  RealType t = 0.; // total time
  RealType t_strain = 0.; // time for which strain is applied.
                          // useful if we decide to use a strain resetting scheme
  ...
  while( t < _TMax && Nt < _NtMax )
  {
    ...
    // Determine cell tensor and update operators
    RealType tmp;
    for (UInt i=0; i<_Dim; i++)
    {
      for (UInt j=0; j<_Dim; j++)
      {
        _CellTensor[i][j] = _InitBoxTensor[i][j];
        tmp = 0.;
        for (UInt k=0; k<_Dim; k++)
          tmp += _InitBoxTensor[i][k] * _G[k][j] * t_strain;

        _CellTensor[i][j] += tmp;
      }
    }

    _CurrGrid->GetCell()->updateCell( _CellTensor );
    ...
    while( phi_err >= _PhiErrTol && phi_iter <= _PhiIterMax )
    {
      ...
      while (!is_physical)
      {
        ...
        is_physical = check_phi(phi_half);
        if ( !is_physical ) 
        { 
          if (_VarDtFlag)
          {
          ...
          {
            std::cout << "*** (ERROR) TimeInt_Model_H_GridDef::outer_loop ***\n";
        ...
        is_physical = check_phi(phi_whole);
        if ( !is_physical )
        {
          if (_VarDtFlag)
          {
    ...
    // --- reset strain ---
    /* if ( t_strain >= 1./_gammadot )
    {
      std::cout << "      --- Resetting strain at time: " << t << " --- " << std::endl;
      Vec2dFieldType Gamma = eye(_Dim, _Dim, 1.);
      for (UInt i=0; i<_Dim; i++)
        for (UInt j=0; j<_Dim; j++)
          Gamma[i][j] -= FieldType(_G[j][i]) * FieldType(t_strain);   

      _PhiOp->def_to_undef( Gamma, _Phi );
      _VelOp->def_to_undef( Gamma, _Vel );
      t_strain = 0.;
    }
    
    // change dt_new so that output is on the exact time we want
    // pick the nearest output time or strain resetting time
    // this has to supercede the existing scheme
    if ( (t+dt_new) > t_check || (t_strain+dt_new) > 1./_gammadot ) 
    { 
      dt_new = std::min(t_check-t, 1./_gammadot-t_strain);
      // if dt < _DtMin, it will crash
      dt_new = std::max(dt_new, _DtMin); 
    } */
...
void TimeInt_ModelH_GridDef :: step_phi( SmartFieldVec &phi, 
...
{ // {{{
  ...
  Vec2dFieldType M_max( _NIcomp, Vec1dFieldType( _NIcomp, FieldType(0.)) );
  M_max = _MobilityModel->get_mobility_max( _Mobility );
...
void TimeInt_ModelH_GridDef :: step_vel( SmartFieldVec const &phi, 
                                 SmartFieldVec &vel, 
                                 SmartFieldVec const &mu,
                                 UInt &iter,
                                 RealType t )
{ // {{{
  ...
  { //{{{
    ...
    vel.setflag_inrealspace( true );
    UInt n = 0; // counter for iteration
    RealType vel_err = 1.;
    UInt vel_iter_max = 500;
...
RealType TimeInt_ModelH_GridDef :: update_timestep( SmartFieldVec & vel, 
                                            RealType phi_err, 
                                            RealType dt )
{ // {{{

...
Model_Energy_FHG_K3 :: Model_Energy_FHG_K3( Operators_Base & OpObj, int ncomp, int nicomp ):
...
Model_Energy_FHG_K3 :: ~Model_Energy_FHG_K3()
{ // {{{
} // }}}

// ----------------- Input/Output -----------------
void Model_Energy_FHG_K3 :: set_params()
{ // {{{
  ...
  if (jsonFile("params_EnergyModel.in"))
  {
    ...
    if ( f2.is_open() )
    {
      ...
      _Kappa.resize(_NIcomp, Vec1dReal(_NIcomp, 0.));
      rapidjson::Value& Kappa_in = d["Kappa"];
      for( int i=0; i < _NIcomp; i++ )
      {
        rapidjson::Value& Kappa_in_2 = Kappa_in[i];
        for( int j=0; j < _NIcomp; j++ )
        {
          _Kappa[i][j] = Kappa_in_2[j].GetDouble();
        }
      }
...
void Model_Energy_FHG_K3::calc_mu(const SmartFieldVec & phi, SmartFieldVec & mu)
{ // {{{ 
  ...
  {
    std::cout << "*** Error in Model_Energy_FHG_K3::calc_mu ***\n";
  ...
  for (int i = 0; i < _NIcomp; i++)
  {
    for (int j = 0; j < _NIcomp; j++)
    {
      _KappaGrad2Phi[j] *= _Kappa[i][j];
      mu[i] -= _KappaGrad2Phi[j];
      _KappaGrad2Phi[j] *= (1./_Kappa[i][j]);
    }
  }
...
void Model_Energy_FHG_K3::calc_mu_lin_exp(const SmartFieldVec & phi, SmartFieldVec & mu_lin)
{ // {{{ 
  ...
  if (phi.getflag_inrealspace() != false)
  {
    std::cout << "*** Error in Model_Energy_FHG_K3::calc_mu_lin_exp ***\n";
    std::cout << "*** phi must be in Fourier space ***\n";
  }
...
void Model_Energy_FHG_K3::calc_mu_lin_imp(const SmartFieldVec & phi, SmartFieldOpMat & F)
{ // {{{ 

  if (_PrecalcMuImp==false)
  {
    std::cout << "*** Error in Model_Energy_FHG_K3::calc_mu_lin_imp ***\n";
  ...
  Vec2dFieldType K(_NIcomp, Vec1dFieldType(_NIcomp, 0.)); 
  for (int i = 0; i < _NIcomp; i++)
  {
    for (int j = 0; j < _NIcomp; j++)
    {
...
void Model_Energy_FHG_K3::calc_Hessian( const SmartFieldVec & phi, SmartFieldMat & Hess )
{ // {{{

  if (phi.getflag_inrealspace() != true)
  {
    std::cout << "*** Error in Model_Energy_FHG_K3::calc_Hessian ***\n";
...
Vec2dFieldType Model_Energy_FHG_K3::calc_Hmax( const SmartFieldVec & phi )
{ // {{{

  if (phi.getflag_inrealspace() != true)
  {
    std::cout << "*** Error in Model_Energy_FHG_K3::calc_Hmax ***\n";
    std::cout << "*** phi must be in Real space ***\n";
  }
...
void Model_Energy_FHG_K3::calc_del_mu(const SmartFieldVec & phi, SmartFieldMat & del_mu)
{ // {{{ 
  ...
  for (int i = 0; i < _NIcomp; i++)
  {
    ...
    {
      _KappaGrad2Phi[j] *= _Kappa[i][j];
      _OpObj->Del_f_ex( _KappaGrad2Phi[j], tmpvec );
      _KappaGrad2Phi[j] *= (1./_Kappa[i][j]);

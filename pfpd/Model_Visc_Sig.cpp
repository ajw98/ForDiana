...
Model_Visc_Sig :: Model_Visc_Sig( int ncomp, int nicomp ):
Model_Visc_Base( ncomp, nicomp )
{ // {{{
} // }}}

Model_Visc_Sig :: ~Model_Visc_Sig()
{ // {{{
} // }}}

// ----------------- Input/Output -----------------
void Model_Visc_Sig :: set_params()
{ // {{{
  ...
  if (jsonFile("params_ViscModel.in"))
  {
    ...
    if ( f1.is_open() )
    {
      ...
      _SigOn.resize(_NIcomp, 0.);
      rapidjson::Value& sigon_in = d["sigon"];
      for( int i=0; i<_NIcomp; i++ )
      {
        _SigOn[i] = sigon_in[i].GetInt();
      }
...
void Model_Visc_Sig :: set_eta( const SmartFieldVec &phi, SmartField &eta )
{ // {{{
  ...
  for (int i=0; i<_NIcomp; i++)
  {
    ...
    {
      tmp = phi[i];
      f_sigmoid(tmp, _PhiT[i], _W[i]); // tmp -> f_sigmoid(tmp)
...
void Model_Visc_Sig :: f_sigmoid( SmartField &phi, RealType phiT, RealType w)
{ // {{{
